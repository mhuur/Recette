...
- **Navigation (refonte Sylva 2026-07)** : plus de sidebar → **topbar sans bandeau** : fond/bordure/titre supprimés, `pointer-events:none` sur la barre (réactivé sur ses enfants), seules restent des pastilles autoportantes — bouton retour (`.topbar-back`, masqué par `body:has(#tab-recipes.active)`, libellé = onglet courant), Courses, avatar → menu Données/Réglages/Déconnexion. Le CSS d'état doit se baser sur l'onglet affiché, **pas** sur `.tab-btn.active` : `switchTab()` n'est pas appelé au boot. **Courses = tiroir latéral** (`body.cart-open`, `openCartDrawer()` ; `switchTab('shopping')` route vers le tiroir). **Debug fusionné dans Réglages** (carte « Notes & remarques », `renderDebug()` rendu au `switchTab('settings')` ; `switchTab('debug')` → settings). Onglets restants : `tab-recipes`, `tab-data`, `tab-settings`.
- **Recettes** : liste en **lignes** (`renderRecipes` → `.recipe-row`) + **panneau détail** à droite (`detailRecipeId`, `body.detail-open`, `renderRecipeDetail()` ; volet superposé < 1100px). Actions (Modifier/Dupliquer/Supprimer/Courses) via le ⋮ du panneau (`openRecipeMenu`). **Filtres, 3 modes** selon la largeur : **rail docké** en colonne gauche ≥1400px (`filter-rail-on` reposé sur `body`, `isFilterRailDocked()`, croix/footer/bouton « Filtres » masqués), **popover** ancré au bouton 769–1399px, **bottom-sheet** ≤768px. Seuil **1400** à garder synchronisé entre le `@media` et `isFilterRailDocked()` : en dessous, la liste tombait sous ~500px et la toolbar repassait sur 2 lignes. Le panneau vit dans `.recipes-layout` (et non plus en fin de `<body>`) pour être une vraie colonne. En rail, l'onglet sort du gabarit centré (`main > #tab-recipes { max-width: 1800px }`, sinon 1180px → liste écrasée et marges vides) et `.recipe-detail` devient fluide (`clamp`). Piège : le bloc `@media (min-width:1400px)` doit venir **après** `(min-width:769px)` **et après** les règles `.filter-sheet-close/-footer` (même spécificité, sinon le masquage saute), et cibler aussi `.filter-sheet-overlay.active`.
- **Bouton retour Android** : `BACK_LAYERS` + `initBackGuard()` (fin du script, avant l'init). Chaque couche ouverte (onglet ≠ Recettes, détail, tiroir, filtres, modale, menu) pousse une entrée d'historique fictive ; `popstate` ferme la couche du dessus, une fermeture par l'UI rend l'entrée (`history.go(-n)`). Synchro par un listener `click` (bubble) **+** MutationObserver → une nouvelle couche se branche en ajoutant une entrée à `BACK_LAYERS` (+ l'élément observé si sa classe n'est pas sur `body`), jamais en modifiant les `open*`/`close*`. **Piège** : Chrome Android ignore au retour les entrées poussées hors geste utilisateur — le `pushState` doit rester **synchrone dans le clic** (l'observer seul, en microtask, ne suffit pas ; un handler qui coupe la propagation, comme le ⋮, doit appeler `syncBackGuard()` lui-même).
...
- **Cartes en surimpression = fond forêt visible, pas le texte de l'app** : les encadrés sont translucides. Pour voir le **vrai fond forêt** derrière une carte (et non un aplat) sans que le texte de la liste transparaisse, on garde le scrim **transparent** ET on **masque l'UI derrière** en `visibility:hidden` :
  - Modale (`.modal-overlay`, tous formats) → `body:has(.modal-overlay.active)` masque `.app-layout` + `.cart-drawer` + `.cart-scrim`.
