...
- **Filtres Chaud/froid et Végétarien** : `temperature` est un **champ de recette** (`'chaud'|'froid'|null`, chips dans la modale) → à maintenir dans l'objet reconstruit par l'enregistrement. `vegetarianOnly` / `pescatarianOnly` sont **calculés** depuis les familles d'ingrédients (`recipeHasFamilies`) : végétarien = ni `MEAT_FAMILIES` (viande, volaille, charcuterie) ni `FISH_FAMILIES` (poisson, fruits de mer) ; pescarien = pas de `MEAT_FAMILIES`. Un ingrédient **sans famille** passe pour compatible avec les deux → la justesse dépend du classement (bouton liste sur chaque famille, cf. Données). Le filtre **Familles est retiré de l'UI** (les familles restent en données, elles alimentent ce calcul) et un filtre persisté est purgé au `load()`.
