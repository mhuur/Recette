...
- **Recettes** : liste en **lignes** (`renderRecipes` → `.recipe-row`) + **panneau détail** à droite (`detailRecipeId`, `body.detail-open`, `renderRecipeDetail()` ; volet superposé < 1100px). Actions (Modifier/Dupliquer/Supprimer/Courses) via le ⋮ du panneau (`openRecipeMenu`). **Filtres, 3 modes** selon la largeur : **rail docké** en colonne gauche ≥1280px (`filter-rail-on` reposé sur `body`, `isFilterRailDocked()`, croix/footer/bouton « Filtres » masqués), **popover** ancré au bouton 769–1279px, **bottom-sheet** ≤768px. Le panneau vit dans `.recipes-layout` (et non plus en fin de `<body>`) pour être une vraie colonne. Piège : la règle `@media (min-width:1280px)` doit venir **après** celle `(min-width:769px)` (même spécificité) et cibler aussi `.filter-sheet-overlay.active`.
