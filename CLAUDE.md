...
- **Rail de filtres = sections à hauteur constante** : une section qui grandit/rétrécit selon la sélection décale tout ce qui est en dessous (plainte utilisateur). « Type de plat » est donc un **bouton d'une ligne** (`#filter-meal-btn`, libellé ellipsé : nom · « A, B » · « N types ») + **menu flottant** `#filter-meal-menu` — enfant direct de `body`, `position:fixed`, `--menu-fill` — et non un nuage de chips (le faceting en retirait, la ligne de chips sélectionnés et « Voir tout » en ajoutaient). Le menu liste **tous** les types, les inatteignables grisés (`.off`) au lieu d'être retirés : la liste ne saute pas sous le doigt. Comme tout menu appendu à `body`, il doit être exclu du clic-extérieur du popover de filtres **et** inscrit dans `BACK_LAYERS` + l'observer. **Menu ouvert = fond figé**, comme un `<select>` natif : `wheel`/`touchmove` hors du menu sont `preventDefault()`. Les deux autres pistes ont été essayées et rejetées par l'utilisateur — le faire suivre son ancre (`position:fixed`) le collait en haut de l'écran une fois le bouton hors champ, et le fermer au scroll le faisait disparaître sous la molette. Sa liste garde `overscroll-behavior:contain` (sinon la molette enchaîne sur le rail en fin de liste).
