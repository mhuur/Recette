...
- **Suggestions = 2e tiroir** (`body.sugg-open`, `openSuggestDrawer()`, bouton « Idées » de la barre) : même ossature CSS que Courses (scrim transparent, UI derrière en `visibility:hidden`, carte centrée de 620px ≥1100px), inscrit dans `BACK_LAYERS`.
  ...
  - **Le tirage survit à la fermeture du tiroir** : `openSuggestDrawer()` ne vide plus `suggItems`, il purge seulement les recettes disparues ou passées en sélection puis rejoue `rerollSuggestions('all')` (qui garde les épinglées). Épingler est un choix explicite : ni un changement de filtre, ni « ignorer les filtres », ni la réduction du curseur ne doivent le défaire (le `slice()` du curseur sacrifiait les épingles en bas de liste). Seul « Ajouter à ma sélection » remet `suggItems = []`, le tirage étant consommé.
  - **Actions filtres = menu déroulant** (`#sugg-filters-btn` dans l'en-tête, à gauche de la croix → `#sugg-menu`), pas des boutons dans la carte d'outils : deux boutons pleine largeur repoussaient la 1re recette hors de l'écran sur mobile. Le menu est **enfant de `body`** (dans le cadre il serait prisonnier de son contexte d'empilement, donc sous le tiroir) et réutilise `.user-menu-item`/`.um-ico`.
    - Deux entrées à conditions distinctes, pilotées par `syncSuggFilterMenu()` : « Modifier les filtres » (`#sugg-open-filters`) seulement si le rail n'est **pas** docké — sous 1280px la `.topbar` est en `visibility:hidden`, donc `#btn-toggle-filter` est injoignable et c'est le **seul** accès aux filtres ; « Ignorer les filtres » (`#sugg-ignore-filters`) seulement si les filtres restreignent vraiment. Aucune des deux → le bouton disparaît au lieu d'ouvrir un menu vide. Appelée par `renderSuggestions()` **et** `updateFilterRail()` (le dock change au redimensionnement sans repasser par un rendu).
    - Câblages obligatoires, chacun déjà oublié une fois : exclure le bouton du clic-extérieur du popover de filtres ; l'utiliser comme **ancre** dans `positionFilterSheet()` (pas l'entrée de menu, qui vient d'être refermée) ; l'inscrire dans `BACK_LAYERS` + l'observer ; fermer le menu dans `closeSuggestDrawer()` (appendu à `body`, il survivrait au tiroir).
    - ⚠️ **Capture headless** : `updateFilterRail()` ferme le menu, et Chrome émet un `resize` après le parsing → le menu est refermé avant le cliché. Neutraliser `window.closeSuggMenu` dans la sonde. Et ne pas combiner `--screenshot` avec `--virtual-time-budget` : Chrome ne rend jamais la main (`--dump-dom` avec, `--screenshot` sans).
