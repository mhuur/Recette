...
- **PWA** (`manifest.webmanifest`, `sw.js`, `icons/`) : stale-while-revalidate shell/CDN, **bypass total Firestore/Auth**. Bumper `CACHE_VERSION` à chaque déploiement et **ajouter tout nouveau fichier servi à `PRECACHE_URLS`**. Icônes = `chef-hat` Lucide crème sur vert forêt, source `icons/icon.svg`, PNG régénérables par capture Chrome headless.
