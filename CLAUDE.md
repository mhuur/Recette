...
- **Suggestions = 2e tiroir** (`body.sugg-open`, `openSuggestDrawer()`, bouton « Idées » de la barre) : même ossature CSS que Courses (scrim transparent, UI derrière en `visibility:hidden`, carte centrée de 620px ≥1100px), inscrit dans `BACK_LAYERS`.
  ...
  - Puise dans `suggPool()` = `getFilteredRecipes()`, ou tout le carnet si `suggIgnoreFilters` : **les filtres du rail s'appliquent par défaut**. Le sous-titre les chiffre (« 7 recettes où piocher sur 382 · filtres du rail appliqués ») et un bouton bascule les ignore — un filtre ingrédient oublié réduisait le tirage à 7 recettes toutes identiques sans que la cause soit lisible. `suggIgnoreFilters` retombe à `false` à chaque ouverture : le rail reste la référence.
