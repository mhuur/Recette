...
- **Voiles opaques derrière les cartes** : les encadrés sont translucides → un voile transparent laissait transparaître le texte de la liste (moche sur mobile). Tous les scrims (`.modal-overlay`, `.detail-scrim`, `.cart-scrim`, `.filter-sheet-overlay` mobile) utilisent `--scrim-veil` (dégradé forêt **opaque**) : la carte montre un fond coloré propre, jamais le texte dessous. `--scrim-veil` n'est **pas** touché par `applyAppearance()` (reste opaque même encadrés très transparents). Ne pas rendre un scrim translucide.
