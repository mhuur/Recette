...
- **PWA** (`manifest.webmanifest`, `sw.js`, `icons/`) : stale-while-revalidate shell/CDN, **bypass total Firestore/Auth**. Bumper `CACHE_VERSION` à chaque déploiement et **ajouter tout nouveau fichier servi à `PRECACHE_URLS`**. Icônes = flamme façon Calcifer, **seule** (les bûches ont été retirées le 2026-07-22 — choix utilisateur) ; source `icons/icon-v4.svg`, déclinaisons `icon-v4-{192,512,apple,maskable-512}.png`.
  - **Fond transparent sur les 4 fichiers**, y compris `apple-touch-icon` et `maskable` (choix utilisateur du 2026-07-22, en connaissance de la contrepartie : iOS comble la transparence en **noir** sur l'écran d'accueil, Android rend un maskable non plein de façon variable). Ne pas « corriger » en remettant un aplat. Capture transparente = `--default-background-color=00000000` ; côté redimensionnement, créer le `Bitmap` en `Format32bppArgb` sinon l'alpha est perdu.
  - Rendu du SVG en headless : le charger **via un wrapper HTML** (`<img src="icon.svg">` + `body{background:transparent}`), pas en `file://` direct — Chrome n'écrit alors aucun screenshot. Ajouter `--user-data-dir=C:\Temp\...` : sans profil dédié le screenshot échoue silencieusement quand une autre instance tourne.
  ...
  - Le PNG `maskable` doit garder ~20% de marge (Android rogne) : il est produit en redessinant le rendu 512 à `scale 0.74` centré sur un canvas transparent (pas besoin d'un 2e SVG).
...
- **Suggestions = 2e tiroir** (`body.sugg-open`, `openSuggestDrawer()`, bouton « Idées » de la barre) : même ossature CSS que Courses (scrim transparent, UI derrière en `visibility:hidden`, carte centrée de 620px ≥1100px), inscrit dans `BACK_LAYERS`.
  ...
  - Puise dans `suggPool()` = `suggRailFiltered()` : **les filtres du rail s'appliquent, toujours**. ⚠️ **Pas `getFilteredRecipes()`** : `recipeMatches()` y applique aussi la **recherche** de la barre du haut et `selectedOnly`, que le rail ne compte pas dans « filtres actifs » et que la barre masquée rend invisibles tiroir ouvert → une recherche oubliée restreignait le tirage sans que rien ne l'indique (d'où `{ search: true, selected: true }` en `skip`).
  ...
  - **Le tiroir est un enfant de `.recipes-layout`**, pas de `body` : ≥1280px il se pose en `position:absolute` (`top/bottom:0; left:calc(224px + 16px)`), donc **aligné au pixel sur le rail** (mêmes haut et bas). ≥1500px il s'arrête avant la colonne détail (`right:calc(392px + 16px)`) pour ne pas être trop large ; en dessous il prend liste + détail (`right:0`), sinon la colonne des cartes deviendrait étroite. Flottant au milieu du cadre, il n'avait de rapport ni avec le rail ni avec le cadre, et tronquait les noms. Sous 1280px il redevient un tiroir latéral `fixed`.
  - **Pas de scrim** : il ne fermait plus rien et interceptait les clics du rail. ⚠️ Un `z-index` sur le rail **ne suffit pas** à le faire repasser devant — il est enfermé dans le contexte d'empilement du cadre, donc sous tout voile en `position:fixed`. `suggSyncWithFilters()`, appelée en fin de `renderRecipes()`, rejoue le tirage **seulement** si l'empreinte des filtres a changé.
  - Corps en grille **`280px | 1fr`** avec `grid-template-areas` — outils et bouton d'ajout à gauche, liste au centre, comme le tiroir Courses. ⚠️ `.sugg-foot` suit `.sugg-main` **dans le DOM** (ordre voulu en mobile : outils, liste, bouton) ; c'est la grille qui le remonte à gauche en desktop. `.sugg-card-name` **passe à la ligne** (mêmes titres longs que `.serving-name`).
  - Badge **« à tester »** réservé aux recettes **sans note ET sans `lastPickedAt`**. L'ancien « jamais faite » sur toute recette sans `lastPickedAt` était faux : ce champ ne remonte qu'à l'arrivée des suggestions, alors qu'une note prouve que la recette a déjà été cuisinée. Le sous-titre chiffre le vivier et **nomme ce qui le restreint** (« 7 recettes où piocher sur 382 · pescarien ») : un filtre ingrédient oublié réduisait le tirage à 7 recettes toutes identiques sans que la cause soit lisible nulle part.
  - **Le tirage survit à la fermeture du tiroir** : `openSuggestDrawer()` ne vide plus `suggItems`, il purge seulement les recettes disparues ou passées en sélection puis rejoue `rerollSuggestions('all')` (qui garde les épinglées). Épingler est un choix explicite : ni un changement de filtre, ni la réduction du curseur ne doivent le défaire (le `slice()` du curseur sacrifiait les épingles en bas de liste). Seul « Ajouter à ma sélection » remet `suggItems = []`, le tirage étant consommé.
  - **Accès aux filtres = bouton rond dans l'en-tête** (`#sugg-filters-btn`, à gauche de la croix) qui ouvre/ferme directement `#filter-sheet-overlay` — pas un bouton dans la carte d'outils, qui repoussait la 1re recette hors de l'écran sur mobile. Masqué quand le rail est docké (`syncSuggFilterBtn()`, appelée par `renderSuggestions()` **et** `updateFilterRail()` — le dock change au redimensionnement sans repasser par un rendu). Sous 1280px la `.topbar` est en `visibility:hidden`, donc `#btn-toggle-filter` est injoignable : c'est le **seul** accès aux filtres. Deux câblages : l'exclure du clic-extérieur du popover, et l'utiliser comme **ancre** dans `positionFilterSheet()`.
  - **Pas de « ignorer les filtres »** (`suggIgnoreFilters`, retiré le 2026-07-22, avec son menu déroulant devenu inutile à une seule entrée) : lu comme un doublon de « Tout effacer » du rail. Le rail fait donc toujours foi, et le sous-titre nomme ce qui restreint le tirage. ⚠️ Ne pas le réintroduire sans arbitrage : la distinction (réversible et limité au tirage, vs `resetAllFilters()` destructif, persisté et qui change aussi la liste de l'onglet) n'a pas paru valoir une entrée d'UI.
  - ⚠️ **Capture headless d'un état ouvert** : ne pas combiner `--screenshot` et `--virtual-time-budget` — Chrome ne rend jamais la main (`--dump-dom` avec, `--screenshot` sans). Et `--user-data-dir` est obligatoire, mais un dossier de profil neuf échoue souvent : réutiliser celui qui a déjà servi.
...
- **Végétarien / pescarien** : **calculés** depuis les familles (`recipeHasFamilies`) — végétarien = ni `MEAT_FAMILIES` ni `FISH_FAMILIES` ; pescarien = `FISH_FAMILIES` **exigé** et `MEAT_FAMILIES` interdit (demande utilisateur du 2026-07-22 : le filtre sert à trouver un plat de poisson, l'ancienne définition « pas de viande » sortait aussi les salades). Les deux critères sont donc **disjoints**, plus emboîtés ; UI = **bulles exclusives** (`#filter-diet .chip-toggle`), et `buildFilterBar()` normalise un état persisté qui aurait les deux (sinon 0 recette). ⚠️ Le libellé reste « Pescarien » alors que le filtre veut du poisson — ne pas « recorriger » `isPescatarianRecipe()` en lisant son seul nom. Le lien ingrédient→famille est `ing.familyIds` (**tableau**), pas `familyId`. Un ingrédient **sans famille** passe pour compatible avec les deux → la justesse dépend du classement. Le filtre **Familles est retiré de l'UI** (les familles restent en données) ; un filtre persisté est purgé au `load()`.
