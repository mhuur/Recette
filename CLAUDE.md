...
- **Détail permanent** ≥1280px : `isDetailPermanent()`, `ensureDetailSelection()` (appelée en fin de `renderRecipes()`) sélectionne la 1re recette filtrée ; `closeRecipeDetail()` est inopérant dans ce mode. Corps en 2 colonnes (`.recipe-detail-cols`) — annuler `margin-top` entre `.recipe-section` en grille, sinon la 2e colonne descend.
