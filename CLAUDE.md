...
- **Suggestions = 2e tiroir** (`body.sugg-open`, `openSuggestDrawer()`, bouton « Idées » de la barre) : même ossature CSS que Courses (scrim transparent, UI derrière en `visibility:hidden`, carte centrée de 620px ≥1100px), inscrit dans `BACK_LAYERS`.
  ...
  - **Bouton « Modifier les filtres » dans le tiroir** (`#sugg-open-filters`, masqué ≥1280px où le rail est docké à côté) : sous 1280px la `.topbar` est en `visibility:hidden`, donc `#btn-toggle-filter` était injoignable — aucun moyen de toucher aux filtres depuis les Idées. Il ouvre le même `#filter-sheet-overlay` (z-index 280 > tiroir 250, et aucune règle ne le masque sous `body.sugg-open`, contrairement à `body.cart-open`). Deux câblages obligatoires : l'exclure du clic-extérieur desktop, et l'utiliser comme **ancre** du popover dans `positionFilterSheet()` (sinon il s'ouvre sous un bouton invisible à l'autre bout de l'écran).
