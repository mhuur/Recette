...
- **Suggestions = 2e tiroir** (`body.sugg-open`, `openSuggestDrawer()`, bouton « Idées » de la barre) : même ossature CSS que Courses (scrim transparent, UI derrière en `visibility:hidden`, carte centrée de 620px ≥1100px), inscrit dans `BACK_LAYERS`.
  ...
  - **Le tiroir est un enfant de `.recipes-layout`**, pas de `body` : ≥1280px il se pose en `position:absolute` sur la zone liste + détail (`top/bottom:0; left:calc(224px + 16px); right:0`), donc **aligné au pixel sur le rail** (mêmes haut et bas) et large de toute la place disponible. Flottant au milieu du cadre, il n'avait de rapport ni avec le rail ni avec le cadre, et tronquait les noms. Sous 1280px il redevient un tiroir latéral `fixed`.
