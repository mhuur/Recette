...
- **Filtres à sélection multiple** : dans un groupe, les valeurs cochées s'**unissent** (OU) et les groupes se croisent (ET). `ratingSel[]` (`'unrated'` + seuils 6/7/8/9 ; plusieurs seuils → le plus permissif) et `temperatures[]`. Temps (`prepMax`/`cookMax`) reste **exclusif** : un OU de « ≤ X » revient au plus grand.
