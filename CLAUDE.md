...
- **Suggestions = 2e tiroir** (`body.sugg-open`, `openSuggestDrawer()`, bouton « Idées » de la barre) : même ossature CSS que Courses (scrim transparent, UI derrière en `visibility:hidden`, carte centrée de 620px ≥1100px), inscrit dans `BACK_LAYERS`.
  ...
  - **Le rail docké reste manipulable tiroir ouvert** (`body.filter-rail-on.sugg-open .filter-sheet-overlay` passe en `position:relative; z-index:245`, au-dessus du scrim 240) ; `suggSyncWithFilters()`, appelée en fin de `renderRecipes()`, rejoue le tirage **seulement** si l'empreinte des filtres a changé. ⚠️ Ne pas viser `.filter-sheet-overlay` sans `filter-rail-on` : entre 1100 et 1280px c'est un popover flottant.
  - Desktop : carte de 760px à **hauteur réglée sur le contenu** (`height:auto; max-height:min(80vh,820px)`, centrée par `translateY(-50%)`) — la pleine hauteur paraissait étroite et vide sous 4 cartes, et laissait le rail dans son ombre.
