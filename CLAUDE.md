...
- **PWA** (`manifest.webmanifest`, `sw.js`, `icons/`) : stale-while-revalidate shell/CDN, **bypass total Firestore/Auth**. Bumper `CACHE_VERSION` à chaque déploiement et **ajouter tout nouveau fichier servi à `PRECACHE_URLS`**. Icônes = flamme façon Calcifer sur bûches, fond vert forêt (fan art maison, usage perso) ; source `icons/icon-v2.svg`, déclinaisons `icon-v2-{192,512,apple,maskable-512}.png`.
...
- **Suggestions = 2e tiroir** (`body.sugg-open`, `openSuggestDrawer()`, bouton « Idées » de la barre) : même ossature CSS que Courses (scrim transparent, UI derrière en `visibility:hidden`, carte centrée de 620px ≥1100px), inscrit dans `BACK_LAYERS`.
  ...
  - Puise dans `getFilteredRecipes()` : **les filtres du rail s'appliquent**. Le sous-titre signale « (filtres actifs) » seulement s'ils retirent vraiment des recettes, sinon on ne comprend pas pourquoi la moitié du carnet ne sort jamais.
